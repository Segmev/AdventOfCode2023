...
pub fn start(file_content: &str) {
    ...
    println!(
        ...
        unsafe_bricks
            .iter()
            .map(|b| simulation(&bricks, b))
            .sum::<usize>()
    );
}

fn simulation(bricks: &Vec<BrickRange>, unsafe_brick: &usize) -> usize {
    let mut acc = 0;
    let dup_bricks = bricks
        .iter()
        .filter(|x| x.id != *unsafe_brick)
        .collect::<Vec<_>>();

    let mut dup_bricks_final: Vec<BrickRange> = vec![];
    for brick in dup_bricks.iter() {
        let mut colliding_bricks = dup_bricks_final
            .iter()
            .filter(|b| b.in_zone(brick))
            .collect::<Vec<_>>();
        ...
        if brick.range_z.start() != final_brick.range_z.start() {
            acc += 1;
        }
        dup_bricks_final.push(final_brick);
    }
    acc
}
